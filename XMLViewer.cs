...
namespace XmlEditor
{
    ...
    {
        Form1 frm1; 
        private XMLViewerSetting settings;
        /// <summary>
        /// The format settings.
        /// </summary>
        public XMLViewerSetting Settings
        {
            get
            {
                if (settings == null)
                {
                    settings = new XMLViewerSetting
                    {
...
//                {1}}}";
                ...
                string rtfFormat = @"{{\rtf1\ansi\ansicpg1252\deflang1033\deflangfe2052{{\fonttbl{{\fs10 Courier New;}}}}{{\colortbl ;{0}}}\viewkind4\uc1\pard\lang1033\f0{1}}}";
                ...
                if (includeDeclaration && xmlDoc.Declaration != null)
                {
                    ...
                    xmlRtfContent.AppendFormat(@"\cf{0} <?\cf{1} xml \cf{2} version\cf{0} =\cf0 ""\cf{3} {4}\cf0 "" \cf{2} encoding\cf{0} =\cf0 ""\cf{3} {5}\cf0 ""\cf{0} ?>\par",
                        XMLViewerSetting.TagID,
                        XMLViewerSetting.ElementID,
                        XMLViewerSetting.AttributeKeyID,
                        XMLViewerSetting.AttributeValueID,
                        xmlDoc.Declaration.Version,
                        xmlDoc.Declaration.Encoding).ToString().TrimStart(); 
                }
                ...
                this.Rtf = string.Format(rtfFormat, Settings.ToRtfFormatString(),
                    xmlRtfContent.ToString().TrimStart());
                //this.Rtf = this.Rtf.Trim();
            }
        ...
        private string ProcessElement(XElement element, int level)
        {
            ...
            string indent = new string(' ', 3 * level); //4
...
{0}\cf{1} </\cf{2} {3}\cf{1} >\par",
                ...
                else
                {
                    childElementsRtfContent.AppendFormat(@"{0}\cf{1} {2}\par",
                        new string(' ', 3 * (level + 1)), //4
                        XMLViewerSetting.ValueID,
